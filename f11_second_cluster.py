...
def main():
    ...
    selected_features = [
        # 1. 強力行為特徵 (最重要)
        'mobile_bank_ratio',   # [新增] 差異最大 (21% vs 65%)，代表操作習慣
        'channel_entropy',     # 差異 14 倍，代表操作複雜度
        'txn_cnt',             # 差異 3 倍，代表頻率

        # 2. 金流特徵
        'total_amt',           # [新增] 差異 2 倍，取代 mean_amt
        'std_amt',             # 差異 2.3 倍，代表金額不穩定
        'max_amt',             # 差異顯著，代表偶爾的大額轉出

        # 3. 時間與急迫性特徵
        'active_days',         # 差異 2.5 倍，代表勤勞程度
        'night_cnt',           # [修改] 建議用次數(night_cnt)而非佔比，因為差異較明顯 (1.1次 vs 3.8次)
        'min_txn_gap',         # 差異 6 倍，抓機器人/連點最準
        'std_txn_gap',         # 抓規律性腳本
        'max_txn_per_day'      # 抓爆發性操作
    ]
    ...
    # 找出 Cluster 0 的資料索引
    mask_c0 = (df['cluster_group'] == 1)
    count_c0 = mask_c0.sum()

    if count_c0 > 0:
        print(f"偵測到 Cluster 6 共有 {count_c0} 筆資料。")
        
        while True:
            sub_input = input(f"👉 [第二層] 請輸入 Cluster 6 要拆成幾群 (預設 5): ").strip()
            if sub_input == "":
                sub_K = 5
                break
        ...
        print(f"🚀 正在對 Cluster 6 執行二次分群 (sub_K={sub_K})...")
        
        # 取得 Cluster 6 的特徵子集 (使用已經標準化過的數據)
        X_c0 = X_scaled[mask_c0]
        ...
        new_labels = [f"1-{l}" for l in sub_labels]
    ...
    plt.title(f'K-Means (Main K={K}, Sub K={sub_K if count_c0>0 else 0})\nTotal Variance: {pc1_var + pc2_var:.1f}%')
